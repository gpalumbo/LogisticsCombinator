...
EXAMPLE 2: Injecting a group
-----------------------------
local entity = ...  -- cargo landing pad
local combinator_id = 12345

-- Check if group already exists
local has_group = logistics_utils.has_logistics_group(entity, "fuel-request")

if not has_group then
  local section_idx, err = logistics_utils.inject_logistics_group(
    entity,
    "fuel-request",
    combinator_id
  )
...
if removed then
  game.print("Removed " .. count .. " sections")
  -- IMPORTANT: Caller must update tracking
  for i = #entity.logistic_sections + 1, #tracking do
    tracking[i] = nil  -- Clean up tracking for removed sections
  end
end


EXAMPLE 4: Cleanup on combinator removal
-----------------------------------------
local combinator_id = 12345
...
TEST CASE 1: Entity with no logistics capability
-------------------------------------------------
Input: A wall entity (no logistic_sections)
Expected: supports_logistics_control returns false
Expected: All other functions handle gracefully (no crash)


TEST CASE 2: Empty circuit network
-----------------------------------
Input: Circuit network with no connected entities
Expected: find_logistics_entities_on_network returns empty table


TEST CASE 3: Network with mixed entities
-----------------------------------------
Input: Network with inserters (has logistics) and belts (no logistics)
Expected: Only inserters returned


TEST CASE 4: Inject into entity with no groups
-----------------------------------------------
Input: Fresh cargo landing pad, group "test-group"
Expected: Section created at index 1
Expected: entity.logistic_sections[1].group == "test-group"


TEST CASE 5: Inject duplicate group
------------------------------------
Input: Entity already has "test-group", inject "test-group" again
Expected: Creates second section (caller responsible for checking duplicates)


TEST CASE 6: Remove non-existent group
---------------------------------------
Input: Entity has no groups, try to remove "test-group"
Expected: Returns false, 0


TEST CASE 7: Remove group not owned by combinator
--------------------------------------------------
Input: Entity has "test-group" injected by combinator A, try to remove with combinator B
Expected: Returns false, 0 (group not removed)


TEST CASE 8: Cleanup with invalid entity
-----------------------------------------
Input: cleanup_entity_groups with destroyed entity
Expected: Returns 0, no crash


TEST CASE 9: Cleanup combinator with many entities
---------------------------------------------------
Input: Combinator injected groups into 10 entities
Expected: All groups removed, count returned


TEST CASE 10: Section index shifting
-------------------------------------
Input: Entity has 3 sections [A, B, C], remove section 2
Expected: Section indices shift: [A, C] now at indices 1, 2
Expected: Tracking data must be updated by caller
